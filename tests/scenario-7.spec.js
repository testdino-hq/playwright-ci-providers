...
// Demo scenario 7 (@firefox). Deterministically fails on shards 1-2, passes on
// 3-4 and locally. Placed in @firefox so Playwright's contiguous sharding lands it
// in a failing shard; the shard-number gate keeps it correct if the suite shifts.

const FAILING_SHARDS = [1, 2];

test('demo scenario 7 @firefox', async ({ page }) => {
